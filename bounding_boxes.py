...
from gtrs.image_utils import crop_image
...
class AnyPolygon(DebugDrawable):
    def __init__(self, polygon: Any) -> None:
        self.polygon = polygon


class BoundingBox(AnyPolygon):
    def __init__(self, box: cvt.Rect, contours: cvt.MatLike, debug_id: int = 0) -> None:
...
class RotatedBoundingBox(AngledBoundingBox):
    def __init__(self, box: cvt.RotatedRect, contours: cvt.MatLike, debug_id: int = 0) -> None:
    ...
    def is_overlapping_extrapolated(self, other: RotatedBoundingBox, unit_size: float) -> bool:
        ...
        tolerance = constants.tolerance_for_tab_line_detection(unit_size)
